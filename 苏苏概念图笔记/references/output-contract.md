...
- Mermaid Markdown 与图片都是默认交付物。公开／非敏感材料缺少浏览器时必须自动使用 Kroki POST 生成白底 SVG，不得询问或安装 Chromium；最终回复必须直接附上图片，不能只给路径。
- 图片导出优先级为 `SVG > 4× PNG > 2× PNG > 1×截图`。SVG 用于长期保存和任意缩放；约 40 节点的 PNG 正式稿通常接近或超过 5000 像素宽。
- 图片渲染采用三级回退：已有浏览器则本地渲染；无浏览器且内容公开／非敏感则强制使用 Kroki POST；只有敏感内容、隐私不明或无网络时才允许只交付 Mermaid。远程回退不得成为上传敏感信息的理由。
